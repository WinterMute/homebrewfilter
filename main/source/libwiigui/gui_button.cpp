...
void GuiButton::SetIcon(GuiImage* img, int n)
{
	icon[n] = img;
	if(img) img->SetParent(this);
}
void GuiButton::SetIconOver(GuiImage* img, int n)
{
	iconOver[n] = img;
	if(img) img->SetParent(this);
}
void GuiButton::SetIconHold(GuiImage* img, int n)
{
	iconHold[n] = img;
	if(img) img->SetParent(this);
}
void GuiButton::SetIconClick(GuiImage* img, int n)
{
	iconClick[n] = img;
	if(img) img->SetParent(this);
}
...
void GuiButton::Draw()
{
	...
	for(int i=0; i<3; i++)
	{
		if((state == STATE_SELECTED || state == STATE_HELD) && iconOver[i])
			iconOver[i]->Draw();
		else if(icon[i])
			icon[i]->Draw();
	}
